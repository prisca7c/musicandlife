...
export class SchedulingService {
  ...
  async deleteLesson(orgId: string, id: string, actor?: Actor) {
    ...
    if (lesson.attendance) {
      ...
      return this.db.db.transaction(async (tx) => {
        ...
        await this.detachLessonReferences(tx, id);
  ...
  // Every other table that can reference a lesson by id, cleared before the
  // lesson row itself is deleted — without this, deleting a lesson that had
  // ever had a reschedule request filed against it, or already had payroll
  // run for it, hit a raw FK violation (a 500) instead of actually deleting
  // anything.
  //  - lessonRequests.createdLessonId: worth keeping (it's the original
  //    proposed times/who asked) — just detach the now-gone lesson from it.
  //  - rescheduleRequests.lessonId: NOT NULL, so it can't be detached the
  //    same way — the request has no meaning once its lesson is gone, so it
  //    is deleted outright.
  //  - payrollItems.lessonId: nullable — a teacher was already paid for
  //    this, so the payroll record (amount, run, date) must survive; only
  //    the dangling lesson reference is cleared.
  private async detachLessonReferences(tx: Executor, id: string) {
    await tx.update(lessonRequests).set({ createdLessonId: null }).where(eq(lessonRequests.createdLessonId, id));
    await tx.delete(rescheduleRequests).where(eq(rescheduleRequests.lessonId, id));
    await tx.update(payrollItems).set({ lessonId: null }).where(eq(payrollItems.lessonId, id));
  }
  ...
  async deleteLessonSeries(orgId: string, id: string, until: string | undefined, actor?: Actor) {
    ...
    for (const l of candidates) {
      ...
      await this.db.db.transaction(async (tx) => {
        await this.detachLessonReferences(tx, l.id);
  ...
  /**
   * Hard-delete every live lesson on one studio-local calendar day — for an
   * unexpected closure (a burst pipe, a power cut), where the business has
   * already told affected families directly and no per-lesson cancellation
   * email should go out. Unlike cancelLesson, this never calls
   * notifyCancelled. Unlike deleteLessonSeries, it never touches any
   * enrollment's scheduleRule — the series should resume next week as
   * normal, not end.
   *
   * Restricted to today-or-earlier: materializeEnrollment's dedup only
   * checks whether a `lessons` row currently occupies a slot within its
   * rolling [now, now+52wk] window (see RECURRENCE_WINDOW_WEEKS above) —
   * generation is clamped to never go into the past, so a same-day delete is
   * never regenerated by that night's run, but a lesson deleted a day or
   * more AHEAD of its date would be silently recreated by an intervening 2am
   * run before the closure date even arrives. There's no "skip just this
   * occurrence" mechanism to make a future date safe, so this deliberately
   * refuses one.
   */
  async deleteLessonsForDay(orgId: string, date: string, actor: Actor) {
    ...
    const candidates = await this.db.db.query.lessons.findMany({
      ...
      with: { attendance: true, student: { columns: { firstName: true, lastName: true } } },
    });

    let deleted = 0;
    const skipped: { lessonId: string; student: string | null; reason: string }[] = [];
    for (const l of candidates) {
      const studentName = l.student ? `${l.student.firstName} ${l.student.lastName}` : null;
      const billed = await this.db.db.query.invoiceLineItems.findFirst({
        where: eq(invoiceLineItems.lessonId, l.id),
        columns: { id: true },
      });
      if (billed) {
        skipped.push({ lessonId: l.id, student: studentName, reason: 'already on an invoice' });
        continue;
      }
      await this.db.db.transaction(async (tx) => {
        if (l.attendance) await this.attendance.reverseAndClearAttendance(orgId, l.id, tx);
        await this.detachLessonReferences(tx, l.id);
        await tx.delete(lessons).where(eq(lessons.id, l.id));
      });
      deleted++;
